...
export class LivingAppsService {
  ...
  static async getAufgaben(): Promise<Aufgaben[]> {
    ...
    return hydrateRecords(records, 'aufgaben');
  }
  static async getAufgabenEntry(id: string): Promise<Aufgaben | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Aufgaben;
    return hydrateRecords([record], 'aufgaben')[0];
  }
  static async createAufgabenEntry(fields: CreateAufgaben): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.AUFGABEN}/records`, { fields: cleanFieldsForApi(fields as any, 'aufgaben') });
    return { ...data, record_id: data.id };
  }
  static async updateAufgabenEntry(id: string, fields: Partial<CreateAufgaben>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.AUFGABEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'aufgaben') });
    return { ...data, record_id: data.id };
  }
