/**
 * AufgabenDialog — pre-generated create/edit dialog for Aufgaben.
 *
 ...
 * AufgabenDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Aufgaben['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<AufgabenDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Aufgaben, Projekte, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Aufgaben';
...
/** Widened prefill type for AufgabenDialog.defaultValues — see file header. */
export type AufgabenDialogDefaults = Omit<Aufgaben['fields'], 'prioritaet' | 'aufgabe_status'> & {
    prioritaet?: LookupValue | string;
    aufgabe_status?: LookupValue | string;
  };

interface AufgabenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Aufgaben['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: AufgabenDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  prioritaet: LOOKUP_OPTIONS['aufgaben']?.['prioritaet'] ?? [],
  aufgabe_status: LOOKUP_OPTIONS['aufgaben']?.['aufgabe_status'] ?? [],
};
...
export function AufgabenDialog({ open, onClose, onSubmit, defaultValues, recordId, projekteList, enablePhotoScan = true, enablePhotoLocation = true }: AufgabenDialogProps) {
  const [fields, setFields] = useState<Partial<Aufgaben['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['projekt', 'titel', 'prioritaet', 'aufgabe_status'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Aufgaben['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'aufgaben');
      await onSubmit(clean as Aufgaben['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "projekt": string | null, // Display name from Projekte (see <available-records>)\n  "titel": string | null, // Aufgabentitel\n  "aufgabe_beschreibung": string | null, // Beschreibung\n  "prioritaet": LookupValue | null, // Priorität (select one key: "niedrig" | "mittel" | "hoch") mapping: niedrig=Niedrig, mittel=Mittel, hoch=Hoch\n  "faelligkeitsdatum": string | null, // YYYY-MM-DD\n  "aufgabe_status": LookupValue | null, // Status (select one key: "offen" | "in_bearbeitung" | "erledigt") mapping: offen=Offen, in_bearbeitung=In Bearbeitung, erledigt=Erledigt\n  "zustaendig_vorname": string | null, // Vorname\n  "zustaendig_nachname": string | null, // Nachname\n}`;
      ...
      setFields(prev => {
        ...
        if (projektName) {
          const projektMatch = projekteList.find(r => matchName(projektName!, [[r.fields.vorname ?? '', r.fields.nachname ?? ''].filter(Boolean).join(' ')]));
          if (projektMatch) merged['projekt'] = createRecordUrl(APP_IDS.PROJEKTE, projektMatch.record_id);
        }
        return merged as Partial<Aufgaben['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('aufgaben') })
    : t('new_entity', { entity: appLabel('aufgaben') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'aufgabe_beschreibung': (
      <div key="aufgabe_beschreibung" className="space-y-1.5">
    ...
    'prioritaet': (
      <div key="prioritaet" className="space-y-1.5">
        <Label htmlFor="prioritaet">{fieldLabel('aufgaben', 'prioritaet')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('aufgaben', 'prioritaet', 'niedrig') ?? 'Niedrig'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('aufgaben', 'prioritaet', 'mittel') ?? 'Mittel'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('aufgaben', 'prioritaet', 'hoch') ?? 'Hoch'}
          </button>
        </div>
    ...
    'aufgabe_status': (
      <div key="aufgabe_status" className="space-y-1.5">
        <Label htmlFor="aufgabe_status">{fieldLabel('aufgaben', 'aufgabe_status')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('aufgaben', 'aufgabe_status', 'offen') ?? 'Offen'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('aufgaben', 'aufgabe_status', 'in_bearbeitung') ?? 'In Bearbeitung'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('aufgaben', 'aufgabe_status', 'erledigt') ?? 'Erledigt'}
          </button>
        </div>
        {showErrors && !fields.aufgabe_status && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'zustaendig_vorname': (
      <div key="zustaendig_vorname" className="space-y-1.5">
    ...
    'zustaendig_nachname': (
      <div key="zustaendig_nachname" className="space-y-1.5">
  ...
  const FIELD_LABELS: Record<string, string> = {"projekt": "Projekt", "titel": "Aufgabentitel", "aufgabe_beschreibung": "Beschreibung", "prioritaet": "Priorität", "faelligkeitsdatum": "Fälligkeitsdatum", "aufgabe_status": "Status", "zustaendig_vorname": "Vorname", "zustaendig_nachname": "Nachname"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"projekt": {"projektname": "Projektname", "beschreibung": "Beschreibung", "vorname": "Vorname", "nachname": "Nachname", "startdatum": "Startdatum", "enddatum": "Geplantes Enddatum", "status": "Status"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.AUFGABEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
    ...
    {createProjekteOpen && (
      <ProjekteDialog
        ...
        defaultValues={createProjekteInitial
          ? ({ projektname: createProjekteInitial } as any)
          : undefined}
      />
    )}
    </>
