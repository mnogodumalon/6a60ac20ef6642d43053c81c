...
import type { Projekte, LookupValue } from '@/types/app';
...
export type ProjekteDialogDefaults = Omit<Projekte['fields'], 'status'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  status: LOOKUP_OPTIONS['projekte']?.['status'] ?? [],
};
...
export function ProjekteDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: ProjekteDialogProps) {
  ...
  const REQUIRED_FIELDS = ['projektname', 'vorname', 'nachname', 'startdatum', 'status'] as const;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'projektname': (
      <div key="projektname" className="space-y-1.5">
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        <Label htmlFor="status">{fieldLabel('projekte', 'status')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('projekte', 'status', 'geplant') ?? 'Geplant'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('projekte', 'status', 'in_bearbeitung') ?? 'In Bearbeitung'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('projekte', 'status', 'abgeschlossen') ?? 'Abgeschlossen'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('projekte', 'status', 'pausiert') ?? 'Pausiert'}
          </button>
        </div>
  ...
  const FIELD_LABELS: Record<string, string> = {"projektname": "Projektname", "beschreibung": "Beschreibung", "vorname": "Vorname", "nachname": "Nachname", "startdatum": "Startdatum", "enddatum": "Geplantes Enddatum", "status": "Status"};
